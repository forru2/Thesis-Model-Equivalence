...
def test_fexp(
        ...
        y_true_bin, 
        y_true_mult,
        sensitive_features_bin, 
        sensitive_features_mult
        ):
    
    expl_bin = fns.fairness_explorer(model_bin, X_ts_bin, y_true_bin, sensitive_features_bin, fairness_metric)
    expl_mult = fns.fairness_explorer(model_mult, X_ts_mult, y_true_mult, sensitive_features_mult, fairness_metric)
    print('BINARY EXPLAINER: \n', expl_bin)
    for i, el in enumerate(expl_mult):
        print(f'CLASS {i} EXPLAINER: \n', el)
    assert isinstance(expl_bin, float), 'Error: the result for binary classification should be a float'
    assert isinstance(expl_mult, list), 'Error: the result for multiclass classification should be a list'
    for el in expl_mult:
        assert isinstance(el, float), 'Error: every element for multiclass should be a float'
    
#advanced_fairness
def test_af():
    
    af_bin = fns.advanced_fairness(m1_bin, X_ts_bin, y_true_bin, sensitive_features_bin, metric = 'eo')
    af_mult = fns.advanced_fairness(m1_mult, X_ts_mult, y_true_mult, sensitive_features_mult, metric = 'cuae')
    print('ADVANCED FAIRNESS FOR BINARY: \n', af_bin)
    print('ADVANCED FAIRNESS FOR MULTICLASS: \n', af_mult)
    assert isinstance(af_bin, float)
    assert len(af_mult) == len(set(y_true_mult))
    for value in af_mult:
